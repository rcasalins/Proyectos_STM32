...
// Incluyendo este archivo estamos incluyendo el correspondiente GPIOx
#include "stm32f4xx.h"
 ...
 * -La diracción del puerto que se esta utilizando (la referencia al puerto)
 * -La configuración especifica del PIN que se esta utilizando
 */

typedef struct
{
	GPIO_TypeDef		*pGPIOx;			/* !< Diracción del puerto al que el PIN corresponde > */
...
/* 8.4.3 GPIOx_PUPDR (dos bit por cada PIN) */
...
/*
 * 8.4.6 GPIOx_ODR (un bit por PIN) - este es el registro para escribir el estado de
 * un PIN (1 o 0). Este registro puede ser escrito y leído desde el software, pero
 * no garantiza una escritura "atomica", por lo cual es prferible utilizar el registro
 * BSRR.
 */
